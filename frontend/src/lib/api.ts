...
export const projectsApi = {
  ...
  getRepositories: async (projectId: string): Promise<ProjectRepoWithDetails[]> => {
    const response = await makeRequest(
      `/api/projects/${projectId}/repositories`
    );
    return handleApiResponse<ProjectRepoWithDetails[]>(response);
  },

  addRepository: async (
    projectId: string,
    data: CreateProjectRepo
  ): Promise<ProjectRepoWithDetails> => {
    const response = await makeRequest(
      `/api/projects/${projectId}/repositories`,
      {
        method: 'POST',
